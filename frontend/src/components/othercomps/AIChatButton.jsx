...
function AIChatButton() {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      setMessages([{
        id:     msgIdRef.current++,
        text:   'HI THERE! I AM YOUR CLINICAL ASSISTANT. HOW CAN I HELP?',
        sender: 'ai',
      }]);
    }
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { id: aiMsgId, text: aiResponse, sender: 'ai' }]);
      ...
      const errText  = err?.response?.data?.message || 'System connection error. Please try again.';
      setMessages((prev) => [...prev, { id: errMsgId, text: errText, sender: 'ai' }]);
    }
  }, [inputMessage, chatHistory, setLastAIMessage, setAiTasks]);
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[200] select-none font-roboto-slab antialiased">
      {!isOpen ? (
        <button
          onClick={toggleChat}
          className="w-16 h-16 rounded-[2rem] flex items-center justify-center bg-white/30 dark:bg-black/30 backdrop-blur-xl border border-white/30 shadow-[0_8px_32px_rgba(0,0,0,0.2)] hover:scale-105 active:scale-95 transition-all duration-300 group overflow-hidden relative"
        >
        ...
        <div className={`rounded-[2.5rem] flex flex-col overflow-hidden shadow-2xl border border-white/20 backdrop-blur-[24px] bg-white/70 dark:bg-[#0a111a]/70 transition-all duration-500 ${chatDimensions}`}>
