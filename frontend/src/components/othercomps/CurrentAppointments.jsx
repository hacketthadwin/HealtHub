...
import {
  ...
  Hourglass, CreditCard, TriangleAlert
} from 'lucide-react';
...
function CurrentAppointments() {
  ...
  const handleNext = () => setCurrentIndex((prev) => Math.min(requests.length - 1, prev + 1));
  ...
  const statusMeta = current ? (STATUS_META[current.status] || { label: current.status, color: 'bg-gray-100 text-gray-500', icon: Clock }) : null;
  const StatusIcon = statusMeta?.icon || Clock;

  const canPay    = current?.status === 'DOCTOR_ACCEPTED_AWAITING_PAYMENT';
  const canCancel = current?.status === 'PENDING_DOCTOR_APPROVAL' || current?.status === 'DOCTOR_ACCEPTED_AWAITING_PAYMENT';
      ...
      <div className="flex flex-row justify-between items-center mb-6 relative z-10">
        ...
        {requests.length > 0 && (
          <div className="flex items-center gap-2 bg-[#1F3A4B]/5 dark:bg-white/5 p-1 rounded-full border border-black/5 dark:border-white/5">
            ...
            <button onClick={handleNext} disabled={currentIndex === requests.length - 1}
              className="p-2 rounded-full hover:bg-[#C2F84F] hover:text-[#1F3A4B] transition-all disabled:opacity-10 text-[#1F3A4B] dark:text-[#C2F84F]">
        ...
        <div className="flex-1 flex flex-col justify-between relative z-10">
          ...
          <div className="space-y-2">
            ...
            {current.scheduledTime && (
              <div className="ml-7 flex flex-col gap-0.5">
                ...
                <p className="text-sm font-black text-[#1F3A4B] dark:text-[#FAFDEE]">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-4 gap-3">
            <div className="flex flex-wrap gap-2">
              ...
              {}
              {!canPay && !canCancel && current.status === 'PENDING_DOCTOR_APPROVAL' && (
